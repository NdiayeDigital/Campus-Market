...
async function loadAdminData() {
    ...
    try {
        // Fetch stats
        const { data: users, error: err1 } = await window.supabase.from('profiles').select('*');
        if (err1)
            throw err1;
        const { data: orders, error: err2 } = await window.supabase.from('orders').select('id, seller_id, status, price');
        if (err2)
            throw err2;
        const { data: products, error: err3 } = await window.supabase.from('products').select('id, title, price, seller_id, seller:seller_id(prenom, nom)');
        if (err3)
            console.error("Products fetch error:", err3);
        if (users) {
            ...
            if (elUsers)
                elUsers.innerText = activeStudents.toString();
            ...
            if (elSellers)
                elSellers.innerText = certifiedSellers.toString();
            let totalRevenue = 0;
            ...
            if (orders) {
                ...
                if (elOrders)
                    elOrders.innerText = baseDeliveredOrders + "+";
                ...
                const elOrders = document.getElementById('stat-orders');
                if (elOrders)
                    elOrders.innerText = "150+";
                totalRevenue = 180000;
            }
            const statRev = document.getElementById('stat-revenue');
            if (statRev)
                statRev.innerText = totalRevenue.toLocaleString('fr-FR') + ' FCFA';
            // Baseline: 4.5 average rating
            const elRating = document.getElementById('stat-rating');
            if (elRating)
                elRating.innerHTML = `4.5 <i class="fa-solid fa-star" style="color: #FCD34D; font-size: 1rem;"></i>`;
    ...
    container.innerHTML = pendingList.map(p => `
                <div style="background: #1E293B; border-radius: 16px; padding: 16px; margin-bottom: 16px; border: 1px solid #334155;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                        <h3 style="font-size: 1.1rem; color: white; margin: 0;">${window.escapeHTML(p.prenom)} ${window.escapeHTML(p.nom)}</h3>
                    ...
                    <p style="font-size: 0.9rem; color: #94A3B8; margin-bottom: 16px;"><i class="fa-solid fa-phone"></i> ${window.escapeHTML(p.telephone)}</p>
...
window.updateSellerRole = async function (userId, newRole) {
    try {
        const { data, error } = await window.supabase.from('profiles').update({ role: newRole }).eq('id', userId).select();
        if (error)
            throw error;
...
function renderAdminProducts(products) {
    ...
    container.innerHTML = products.map(p => {
        const sellerName = p.seller ? `${window.escapeHTML(p.seller.prenom)} ${window.escapeHTML(p.seller.nom)}` : 'Inconnu';
        return `
                    <div style="background: #1E293B; border-radius: 16px; padding: 16px; margin-bottom: 16px; border: 1px solid #334155; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <h3 style="font-size: 1rem; color: white; margin: 0; margin-bottom: 4px;">${window.escapeHTML(p.title)}</h3>
...
window.deleteProductAdmin = async function (productId) {
    if (!confirm("Voulez-vous vraiment supprimer ce produit de la plateforme ? Cette action est irréversible."))
        return;
    try {
...
function renderAdminUsers(allUsers) {
    ...
    container.innerHTML = allUsers.map(u => {
        ...
        return `
                    <div style="background: #1E293B; border-radius: 16px; padding: 16px; margin-bottom: 16px; border: 1px solid #334155; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <h3 style="font-size: 1rem; color: white; margin: 0; margin-bottom: 4px;">${window.escapeHTML(u.prenom)} ${window.escapeHTML(u.nom)} ${roleBadge}</h3>
                            <p style="font-size: 0.8rem; color: #94A3B8; margin: 0;"><i class="fa-solid fa-phone"></i> ${window.escapeHTML(u.telephone || 'Non renseigné')}</p>
                        </div>
...
window.deleteUserAdmin = async function (userId) {
    if (!confirm("Voulez-vous vraiment supprimer cet étudiant ? Cette action est irréversible et supprimera son profil."))
        return;
    try {
...
window.loadSellerDashboard = async function () {
    ...
    if (profile && profile.role === 'vendeur') {
        ...
        if (adminProfileNameEl)
            adminProfileNameEl.innerText = window.escapeHTML(profile.prenom) + " " + window.escapeHTML(profile.nom);
        const adminProfilePhoneEl = document.getElementById('admin-profile-phone');
        if (adminProfilePhoneEl)
            adminProfilePhoneEl.innerText = window.escapeHTML(profile.telephone);
        ...
        if (toggleInput)
            toggleInput.checked = openVal;
        if (toggleInputProfil)
            toggleInputProfil.checked = openVal;
        if (toggleInput) {
            if (openVal) {
    ...
    if (!error && orders) {
        ...
        window.lastFetchedOrders = orders;
        // Gérer la répétition de l'alerte sonore pour les commandes en attente
        const hasPending = orders.some(o => o.status === 'pending');
        if (hasPending) {
            if (typeof window.startPendingOrdersSoundLoop === 'function') {
                window.startPendingOrdersSoundLoop();
            }
        }
        else {
            if (typeof window.stopPendingOrdersSoundLoop === 'function') {
                window.stopPendingOrdersSoundLoop();
            }
        }
        let totalRevenue = 0;
        ...
        if (avgEl)
            avgEl.innerText = avgOrder.toLocaleString('fr-FR') + " F";
        ...
        if (convEl)
            convEl.innerText = conversionRate + "%";
        ...
        const catRevenue = { food: 0, tech: 0, fashion: 0, services: 0 };
        const catLabels = { food: 'Restauration', tech: 'Technologie', fashion: 'Mode', services: 'Services' };
        const catColors = { food: '#10B981', tech: '#3B82F6', fashion: '#F59E0B', services: '#8B5CF6' };
        delivered.forEach(o => {
        ...
        const { data: products } = await window.supabase.from('products').select('id, stock').eq('seller_id', user.id);
        let stockPct = 100;
        let inStockCount = 0;
        ...
        const { data: reviews } = await window.supabase.from('reviews').select('rating').eq('seller_id', user.id);
        let avgRating = 5.0;
        let ratingPct = 100;
    ...
    const { data: products } = await window.supabase.from('products').select('*').eq('seller_id', user.id);
    window.lastFetchedProducts = products;
            ...
            prodContainer.innerHTML = products.map(p => {
                ...
                return `
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; border: 1px solid #334155; border-radius: 12px; margin-bottom: 10px; background: #1E293B;">
                        <div>
                            <strong style="display: block; font-size: 0.9rem; color: #F8FAFC;">${window.escapeHTML(p.title)}</strong>
...
function renderTopProducts(orders) {
    ...
    container.innerHTML = sorted.map(([title, qty]) => `
        <div style="display: flex; justify-content: space-between; align-items: center; background: #F8FAFC; border-radius: 8px; padding: 10px 14px; border: 1px solid #E2E8F0; font-size: 0.85rem;">
            <span style="font-weight: 600; color: #1E293B;">${window.escapeHTML(title)}</span>
...
window.toggleSellerStatus = async function () {
    ...
    if (toggleInputProfil)
        toggleInputProfil.checked = isOpen;
    if (isOpen) {
        textSpan.innerText = "Ouvert";
        textSpan.style.color = "#10B981";
        bgDiv.style.background = "#10B981";
    ...
    try {
        const { error } = await window.supabase.from('profiles').update({ is_open: isOpen }).eq('id', user.id);
        if (error)
            throw error;
        if (window.showToast)
            showToast(isOpen ? "Votre boutique est maintenant ouverte" : "Votre boutique est fermée", "success");
    }
    catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const imgInput = document.getElementById('new_prod_image');
    const previewContainer = document.getElementById('new_prod_image_preview_container');
    const previewImg = document.getElementById('new_prod_image_preview');
    ...
    if (addProdForm) {
        addProdForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                addProdForm.reset();
                if (window.clearProductImagePreview) {
...
window.startPendingOrdersSoundLoop = function () {
    if (pendingOrdersInterval)
        return;
    pendingOrdersInterval = setInterval(() => {
        const orders = window.lastFetchedOrders;
        const hasPending = orders && orders.some((o) => o.status === 'pending');
        if (hasPending) {
            playNotificationSound();
            if (window.showToast)
                window.showToast("Vous avez des commandes en attente à traiter ! 🔔", "info");
        }
        else {
            window.stopPendingOrdersSoundLoop();
        }
...
window.stopPendingOrdersSoundLoop = function () {
    if (pendingOrdersInterval) {
    ...
    if (profile.role === 'vendeur') {
        ...
        supabase
            ...
            if (newProfile && newProfile.role === 'vendeur_pending') {
                ...
                if (window.showToast)
                    showToast(`Nouvelle demande vendeur de ${newProfile.prenom} ${newProfile.nom} !`, "info");
...
function handleOrderChange(payload, currentUserId) {
    ...
    if (eventType === 'INSERT' && newOrder.seller_id === currentUserId) {
        ...
        if (typeof loadSellerDashboard === 'function')
            loadSellerDashboard();
    }
    ...
    if (eventType === 'UPDATE' && newOrder.buyer_id === currentUserId) {
        if (!oldOrder || newOrder.status !== oldOrder.status) {
            ...
            if (newOrder.status === 'shipped')
                msg = "Le livreur est en route ! Préparez-vous.";
            if (newOrder.status === 'delivered')
                msg = "Commande livrée. Bon appétit !";
...
let audioCtx = null;
window.enableAudioAlerts = async function () {
    ...
    if (!audioEnabled) {
        try {
            ...
            if (window.showToast)
                window.showToast("Alertes sonores activées ! 🔊");
        }
        catch (e) {
        ...
        if (window.showToast)
            window.showToast("Alertes sonores désactivées. 🔇");
    }
};
...
// ==========================================
window.currentOrderTab = 'pending';
window.lastFetchedOrders = null;
window.lastFetchedProducts = null;
window.switchOrderTab = function (tab) {
    ...
    if (window.lastFetchedOrders) {
        window.renderSellerOrdersList(window.lastFetchedOrders);
    }
};
window.renderSellerOrdersList = function (orders) {
    const listContainer = document.getElementById('seller-orders-list');
    if (!listContainer)
        return;
    // Filter based on active tab
    let filteredOrders = [];
    const currentTab = window.currentOrderTab;
    ...
    if (filteredOrders.length === 0) {
        ...
        if (currentTab === 'processing')
            emptyMsg = "Aucune commande en cours.";
        if (currentTab === 'completed')
            emptyMsg = "Aucun historique disponible.";
        ...
        return `
            <div style="border: 1px solid #334155; border-radius: 12px; padding: 16px; margin-bottom: 12px; background: #1E293B;">
                ...
                <div style="font-size: 0.85rem; margin-bottom: 8px; color: #F8FAFC; line-height: 1.4;">
                    <strong>👤 ${window.escapeHTML(buyerName)}</strong><br>
                    📍 Pavillon / Chambre : ${window.escapeHTML(o.delivery_address)}<br>
                    📞 ${window.escapeHTML(buyerPhone)}
                ...
                <div style="font-size: 0.85rem; color: #94A3B8; margin-bottom: 12px; border-top: 1px dashed #334155; padding-top: 8px;">
                    🛒 ${o.quantity || 1}x <strong>${window.escapeHTML(o.product?.title || 'Produit inconnu')}</strong> (${o.price} F)
                </div>
...
window.updateProductStock = async function (productId, delta) {
    const products = window.lastFetchedProducts;
    if (!products)
        return;
    ...
    try {
        const { error } = await window.supabase.from('products').update({ stock: product.stock }).eq('id', productId);
        if (error)
            throw error;
...
window.clearProductImagePreview = function () {
    const imgInput = document.getElementById('new_prod_image');
    const previewContainer = document.getElementById('new_prod_image_preview_container');
    const previewImg = document.getElementById('new_prod_image_preview');
    if (imgInput)
