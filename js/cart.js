        ...
        html += `
            <div class="cart-item">
                <div class="cart-item-img cart-item-avatar" style="color: ${item.color || '#1D4ED8'};">
                    <i class="fa-solid ${window.escapeHTML(item.icon || 'fa-box')}"></i>
                </div>
                <div class="cart-item-info">
                    <div class="cart-item-name">${window.escapeHTML(item.title)}</div>
...
function savePendingOrdersLocally(ordersToInsert) {
...
async function syncPendingOrders() {
    ...
    try {
        ...
        if (typeof window.fetchMyOrders === 'function') {
        ...
        listContainer.innerHTML = orders.map(o => {
            ...
            <div class="order-item-card" data-status="${o.status}" style="display: block; background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: 16px; margin-bottom: 12px;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span style="font-size: 0.85rem; color: var(--color-text-muted);">${window.escapeHTML(productTitle)}</span>
